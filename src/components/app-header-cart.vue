<template>
   <div class="cart">
     <router-link class="curr" to="/cart">
       <i class="iconfont icon-gouwucheman"><em>{{$store.getters['cart/validTotal']}} </em></i>
     </router-link>
     <div class="layer" v-if="$store.getters['cart/validTotal']>0 && $route.path!=='/cart'">
      <div class="list">
        <div class="item" v-for="(item,index) in $store.getters['cart/validList']" :key="index">
          ...
          <i @click="deleteGood(item.skuId)" class="iconfont icon-chacha"></i>
        </div>
      </div>
      <div class="foot">
        <div class="total">
