<template>
  <XtxDialog title="取消订单" v-model:visible="visibel">
    <!-- 组件内容 -->
    <div class="cancel-info">
     ...
     <!-- 按钮操作 -->
    <template #footer>
      <XtxButton type="gray" @click="visibel=false" style="margin-right:20px">取消</XtxButton>
...
export default {
   setup(){
     ...
     const submit=()=>{
        if(!curText.value) return proxy.$message('warn','请选择取消原因')
        cancelOrder(currOrder.value.id,curText.value).then(res=>{
