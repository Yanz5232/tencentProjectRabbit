    ...
    <div class="container">
      ...
      <div class="pay-info" v-if="order">
        ...
        <div class="tip">
          ...
          <p v-if="order.countdown==-1">订单以超时</p>
        </div>
        <div class="amount">
          <span>应付总额：</span>
      ...
      <div class="pay-type">
        ...
        <div class="item">
          ...
          <a target="_blank" @click="visibleDialog=true" class="btn alipay" :href="payUrl"></a>
        </div>
        <div class="item">
          <p>支付方式</p>
...
export default {
  ...
  setup(){
    ...
    findOrderDetail(route.query.orderId).then(res=>{
       ...
       if(res.result.countdown==-1) return
       start(res.result.countdown)
    }) 
    const {start,timeText}=usePayTime()
    const payUrl=ref('')
    const redirect=encodeURIComponent('http://www.corho.com:8080/#/pay/callback')
    payUrl.value=`${baseURL}pay/aliPay?orderId=${route.query.orderId}&redirect=${redirect}`
    return{
      order,
      timeText,
      payUrl,
